...
use crate::rpc::{RequestHeader, ResponseHeader, Metadata, MessageId};
...
pub trait ServerCodec: Send + Sync {
    ...
    async fn read_request_body<'c>(&'c mut self) -> Option<Result<Box<dyn erased::Deserializer + Send + Sync + 'c>, Error>>;

    async fn write_response(&mut self, header: ResponseHeader, body: &(dyn erased::Serialize + Send + Sync)) -> Result<usize, Error>;
}

#[async_trait]
pub trait ClientCodec: Send + Sync {
    async fn read_response_header(&mut self) -> Option<Result<ResponseHeader, Error>>;
    async fn read_response_body<'c>(&'c mut self) -> Option<Result<Box<dyn erased::Deserializer + Send + Sync + 'c>, Error>>;

    async fn write_request(&mut self, header: RequestHeader, body: &(dyn erased::Serialize + Send + Sync)) -> Result<usize, Error>;
}
    ...
    T: CodecRead + CodecWrite + Send + Sync
{
    ...
    async fn read_request_body<'c>(&'c mut self) -> Option<Result<Box<dyn erased::Deserializer<'c> + Send + Sync + 'c>, Error>> {
    ...
    T: CodecRead + CodecWrite + Send + Sync
{
    ...
    async fn read_response_body<'c>(&'c mut self) -> Option<Result<Box<dyn erased::Deserializer<'c> + Send + Sync + 'c>, Error>> {
        self. read_body().await
    }
